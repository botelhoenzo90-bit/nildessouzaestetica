...
// Salva o pedido de agendamento no painel da clínica como "pendente".
// O horário só é confirmado depois do comprovante do sinal no WhatsApp.
export const saveAppointment = createServerFn({ method: "POST" })
  .inputValidator((data: NewAppointment) => data)
  .handler(async ({ data }): Promise<{ ok: boolean }> => {
    try {
      const supabase = publicClient();
      const { error } = await supabase.from("appointments").insert({ ...data, status: "pendente" });
      return { ok: !error };
    } catch {
      return { ok: false };
    }
